    ...
    def ks_import_xlsx_file(self):
        ...
        for row_index, row in enumerate(map(sheet.row, range(sheet.nrows)), 1):
            if row_index == 1:
                ks_sheet_columns = []
                for cell in row:
                    if cell.value not in ['id', 'ks_target_task_id', 'ks_task_link_type']:
                        ks_sheet_columns.append((cell))
                ...
                updated_task_ids[int(row_id.value)] = created_task.id
        for row_index, row in enumerate(map(sheet.row, range(sheet.nrows)), 1):
            if row_index != 1:
                if row[-2].value:
                    self.env['ks.task.link'].sudo().create({
                        'ks_source_task_id': updated_task_ids[row[0].value],
                        'ks_target_task_id': updated_task_ids[int(float(row[-2].value))],
                        'ks_task_link_type': str(int(float(row[-1].value))),
                    })
